...
import { fetchEconomicSignals } from "@/lib/prediction/domains/economic";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const predictionTypes = await getPredictionTypes("economic");
    const results = [];
    ...
    for (const pt of predictionTypes) {
      try {
        ...
        const dataSources = (pt.data_sources as Array<{ api: string; series?: string; symbol?: string; coin?: string }>) || [];

        // Fetch signals from all configured data sources
        const signals = await fetchEconomicSignals(pt.slug, dataSources, weights);
    ...
    console.error("Economic cron error:", error);
    return NextResponse.json(
      { error: "Failed to refresh economic predictions" },
      { status: 500 }
    );
  }
}
...
async function shouldRefreshPrediction(
  ...
  // If we have no signals in the tier window, it's time to refresh
